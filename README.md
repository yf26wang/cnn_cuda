...
The cuda device code contains two __global__ functions that can be invoked from the host. First, the compute_cnn_part1() function contains the convolution layer, ReLU layer, and the flattening part of the output layer. The function runs with 10 blocks each representing a neuron, and 400 threads used per block representing a 20x20 matrix result. Each thread is responsible for the following:
